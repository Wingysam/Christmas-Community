...
export default function (db) {
  ...
  router.get('/:code', async (req, res) => {
    const row = (await db.allDocs({ include_docs: true })).rows.find(
      ({ doc }) => doc.signupToken === req.params.code,
    )
  ...
  router.post('/:code', async (req, res) => {
    const { doc } = (await db.allDocs({ include_docs: true })).rows.find(
      ({ doc }) => doc.signupToken === req.params.code,
    )

    if (doc.expiry < new Date().getTime())
      return res.redirect(`/confirm-account/${req.params.code}`)
